...
class UserController {
  ...
  async activate(req, res, next) {
    try {
      ...
      return res.redirect(`${process.env.CLIENT_URL}/activation?success=true`);
    } catch (e) {
      if (e.code) return res.redirect(`${process.env.CLIENT_URL}/activation?success=false&errorCode=${e.code}`);
      next(e);
    }
  }
  ...
  async resend(req, res, next) {
    try {
      const { refreshToken } = req.cookies;
      await userService.resend(refreshToken);
