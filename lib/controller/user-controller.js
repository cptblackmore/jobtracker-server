...
class UserController {
  async registration(req, res, next) {
    try {
      ...
      if (!errors.isEmpty()) {
        if (errors.array().some((err) => err.path === "email"))
          throw ApiError.InvalidEmailError();
        if (errors.array().some((err) => err.path === "password"))
  ...
  async activate(req, res, next) {
    try {
      ...
      return res.redirect(
        `${process.env.CLIENT_URL}/activation?code=ACTIVATION_SUCCESS`,
      );
    } catch (e) {
      if (e.code)
        return res.redirect(
          `${process.env.CLIENT_URL}/activation?code=${e.code}`,
        );
      next(e);
    }
  }
  ...
  async refreshAck(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json({ message: "New refresh token saved successfully" });
    } catch (e) {
      next(e);
    }
  }
  ...
  async synchronizeFavorites(req, res, next) {
    try {
      ...
      const synchronizedFavorites = await favoritesService.synchronizeFavorites(
        userId,
        favorites,
      );
  ...
  async updateFavorites(req, res, next) {
    try {
      const userId = req.user.id;
      const favorites = req.body.favorites;
      const updatedFavorites = await favoritesService.updateFavorites(
        userId,
        favorites,
      );
