...
module.exports = function (req, res, next) {
  ...
  const hasPlaceInQuery = Object.values(query).some((value) => {
    ...
    if (Array.isArray(value))
      return value.some((v) => typeof v === "string" && v.startsWith("place~"));
    return false;
  });
  const hasPlaceInTargetUrl =
    typeof targetUrl === "string" && targetUrl.includes("place~");
  ...
  for (const [param, value] of Object.entries(query)) {
    if (typeof value === "string" && value.startsWith("place~")) {
      ...
      const arr = value.filter(
        (v) => typeof v === "string" && v.startsWith("place~"),
      );
      if (arr.length > 0) {
        placeKeys[param] = arr;
      }
  ...
  const resolvePlaceValue = (value) => {
    ...
    if (nameOrId === "name") {
      if (cityOrRegion === "city") {
        ...
        const result = placesService.findRegionSourceIdByName(
          placeValue,
          source,
        );
        if (result) return result;
      } else {
        const result = placesService.findPlaceSourceIdByName(
          placeValue,
          source,
        );
        if (result) return result;
  ...
  try {
    ...
    if (hasPlaceInTargetUrl) {
      ...
      req.headers["x-target-url"] = targetUrl.replace(
        /place~[^/?&#]+(?:~[^/?&#]*){0,2}/g,
        (match) => {
