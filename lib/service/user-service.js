...
async function updatePendingRefreshToken(user) {
  const userDto = new UserDto(user);
  const tokens = await tokenService.generateTokens({ ...userDto });
  await tokenService.savePendingToken(userDto.id, tokens.refreshToken);
...
class UserService {
  async registration(email, password) {
    ...
    const user = await UserModel.create({
      ...
      activationLink,
      nextResendAt: Date.now() + Number(process.env.RESEND_COOLDOWN),
    });
  ...
  async resend(refreshToken) {
    ...
    if (
      nextResendAt &&
      Date.parse(nextResendAt) - Date.now() > process.env.RESEND_COOLDOWN
    ) {
  ...
  async refresh(refreshToken) {
    ...
    return await updatePendingRefreshToken(user);
  }

  async refreshAck(refreshToken) {
    if (!refreshToken) {
      throw ApiError.InvalidRefreshTokenError();
    }
    const userData = await tokenService.validateRefreshToken(refreshToken);
    const pendingTokenFromDb =
      await tokenService.findPendingToken(refreshToken);
    if (!userData || !pendingTokenFromDb) {
      throw ApiError.InvalidRefreshTokenError();
    }
